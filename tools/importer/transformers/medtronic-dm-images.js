...
/**
 * Transformer: Medtronic Dynamic Media / Scene7 images.
 *
 * Detected in migration-work/our-company/metadata.json .images.mapping:
 *   https://medtronic.scene7.com/is/image/Medtronic/...  (Scene7 IS/Image)
 *
 * Rewrites DM/Scene7 <img> tags into anchors so the DM URL round-trips through
 * markdown intact. The companion client-side auto-block in scripts/scripts.js
 * (installed by the site-migration orchestrator, Step 5a) rebuilds them into
 * responsive <picture> elements at render time; the aem.js dispatcher (Step 5b)
 * preserves query params for block decorators that call createOptimizedPicture.
 *
 * Runs in afterTransform ONLY: block parsers run between the hooks and extract
 * <img> references into their cells (cards/carousel/columns), so rewriting imgs
 * to anchors in beforeTransform would leave parsers with empty image cells.
 *
 * Helpers below are copied byte-identical from the canonical dm-scene7-helpers.js
 * (transformer subset: detectDynamicMediaUrl, findLinkedDmCarrier,
 * EMPTY_ALT_SENTINEL, altToLinkText).
 */
