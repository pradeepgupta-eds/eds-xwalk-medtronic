...
export default function decorate(block) {
  ...
  // Drop any leading label/title rows that carry only text (e.g. the block-name
  // "Carousel News" row) — a real slide must contain an image or a heading.
  block.querySelectorAll(':scope > div').forEach((row) => {
    const hasMedia = row.querySelector('picture, img, h1, h2, h3, h4, h5, h6');
    if (!hasMedia) row.remove();
  });
